# ============================================================
# FluidLM — Spectral Pressure Experiment (Exp 17)
# experiments/17_spectral_pressure.py
#
# Araştırma Sorusu (RQ5):
#   Cumsum tabanlı yerel basınç yerine global Helmholtz-Poisson
#   çözümü (FFT) NS routing'i iyileştirir mi?
#
# Exp16'dan tek fark (_rhs):
#   ÖNCE: p = cumsum(-div_adv) / alpha   [yerel, O(L)]
#   SONRA: (∇²-α²)p = -div(adv)          [global, FFT, O(L log L)]
#           p̂_k = -f̂_k / (2(cos(2πk/L)-1) - α²)
#
# Exp16 referans sonucu: best_ppl=99.79 @1B
# GPT-S referans (1B beklenti): ~70-75 PPL
#
# Kullanım:
#   python experiments/17_spectral_pressure.py --tokens 1e9
#   python experiments/17_spectral_pressure.py --tokens 1e9 --no_spectral  # ablasyon: cumsum geri
#
# Çıktı:
#   results/17_spectral_k4_S_s{seed}_best.pt
#   results/17_spectral_pressure.json
# ============================================================
...
parser = argparse.ArgumentParser(description="FluidLM Spectral Pressure (Exp 17)")
parser.add_argument("--multihop_k",   type=int,   default=4,
                    help="Multi-hop gradient ölçeği (exp16 ile aynı default=4)")
parser.add_argument("--no_spectral",  action="store_true",
                    help="Ablasyon: cumsum basıncı kullan (exp16 ile karşılaştırma)")
...
USE_SPECTRAL = not args.no_spectral   # True → FFT Helmholtz, False → cumsum ablasyon
...
pressure_mode = "FFT-Helmholtz" if USE_SPECTRAL else "cumsum (ablasyon)"
master_print(f"[Device] {gpu_name}  VRAM={vram_gb:.0f}GB  dtype={DTYPE}")
master_print(f"[Exp17] pressure={pressure_mode}  multihop_k={args.multihop_k}")
...
def multihop_gradient(u: torch.Tensor, k: int = 4) -> torch.Tensor:
...
def multihop_laplacian(u: torch.Tensor, k: int = 4) -> torch.Tensor:
    scales = _hop_scales(k)
    total  = torch.zeros_like(u)
    for s in scales:
        p     = F.pad(u, (0, 0, 2 * s, 0))
...
def multihop_divergence(u: torch.Tensor, k: int = 4) -> torch.Tensor:
    return multihop_gradient(u, k).mean(dim=-1)

def spectral_pressure(adv: torch.Tensor, alpha: torch.Tensor,
                      hop_k: int = 4) -> torch.Tensor:
    """
    Helmholtz-Poisson: (∇² − α²)p = −div(adv)
    FFT çözümü: p̂_k = −f̂_k / λ_k
                λ_k = 2(cos(2πk/L)−1) − α²
    Multi-hop divergence kullanır (exp16 tutarlılığı için).
    Çıkış: (B, L) basınç alanı, std-normalize edilmiş.
    """
    div  = multihop_divergence(adv, hop_k)             # (B, L)
    B, L = div.shape
    f    = torch.fft.rfft(div.float(), dim=1)           # (B, L//2+1) complex
    ...
    lam  = lam.masked_fill(lam.abs() < 1e-8, -1e-8)   # sıfır bölen önle
    p    = torch.fft.irfft(-f / lam.unsqueeze(0), n=L, dim=1).to(adv.dtype)
    # std normalize: basıncın ölçeğini sabit tut
    return p / (p.detach().std(dim=1, keepdim=True).clamp(min=0.5) + 1e-6)

def cumsum_pressure(adv: torch.Tensor, alpha: torch.Tensor,
                    hop_k: int = 4) -> torch.Tensor:
    """Ablasyon: exp16 ile aynı cumsum tabanlı basınç."""
    div_adv = multihop_divergence(adv, hop_k)
...
class FluidLayer(nn.Module):
    def __init__(self, d_model: int, hop_k: int = 4, use_spectral: bool = True,
    ...
    def _rhs(self, u: torch.Tensor) -> torch.Tensor:
        ...
        # ── Basınç: FFT Helmholtz VEYA cumsum (ablasyon) ──────────────────
        if self.use_spectral:
            # Global: (∇²-α²)p = -div(adv)
            p = spectral_pressure(adv, self.alpha, self.hop_k)   # (B, L)
        else:
            # Yerel: cumsum (exp16 ablasyon)
            p = cumsum_pressure(adv, self.alpha, self.hop_k)     # (B, L)

        p_grad = self.p_scale * multihop_gradient(
                     p.unsqueeze(-1), self.hop_k).expand_as(u)
...
class FluidLM(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, n_layers: int,
                 max_seq_len: int, hop_k: int = 4, use_spectral: bool = True,
        ...
        self.layers    = nn.ModuleList([
            FluidLayer(d_model, hop_k=hop_k, use_spectral=use_spectral,
...
def flops_per_token(model_type: str, d: int, L: int, T: int,
                    vocab: int, mlp_ratio: int,
                    hop_k: int = 4, use_spectral: bool = True) -> dict:
    hidden    = d * mlp_ratio
    ...
    if model_type == "fluid":
        ...
        # FFT: O(L log L) per katman — ama MFLOP/token olarak normalize edince L log L / L = log L
        fft_per_token = int(math.log2(max(T, 2))) * d if use_spectral else 0
        routing_total = (ns_per_token + fft_per_token) * L
        pressure_tag  = "FFT-Helmholtz" if use_spectral else "cumsum"
        routing_label = f"NS v4+multihop-k{hop_k}+{pressure_tag} (scales={_hop_scales(hop_k)})"
        routing_complexity = "O(L·T·D·log(k)+L·T·log(T))" if use_spectral else "O(L·T·D·log(k))"
    else:
        n_heads       = d // 64
        head_dim      = d // n_heads
...
def train_one(model_type: str, scale_cfg: ScaleConfig, seed: int,
    ...
    sc      = scale_cfg
    ptag    = "spectral" if use_spectral else "cumsum"
    run_id  = f"17_{model_type}_{ptag}_k{hop_k}_{sc.name}_s{seed}"
    master_print(f"\n{'='*60}")
    master_print(f"  RUN: {run_id}")
    master_print(f"  Scale: d={sc.d_model}, L={sc.n_layers}, seq={sc.seq_len}")
    master_print(f"  pressure={ptag}  multihop_k={hop_k}  scales={_hop_scales(hop_k)}")
    ...
    flops_info = flops_per_token(
        ...
        vocab_size, args.mlp_ratio, hop_k, use_spectral)
    ...
    if model_type == "fluid" and IS_MASTER:
        ...
        master_print(f"  v (viscosity): early(0-3)={sum(nu_vals[:4])/4:.4f}  "
                     f"late({sc.n_layers-4}-{sc.n_layers-1})={sum(nu_vals[-4:])/4:.4f}")
        master_print(f"  v range: [{min(nu_vals):.4f}, {max(nu_vals):.4f}]  "
                     f"gradient={(nu_vals[-1]-nu_vals[0]):.4f}")
        # α exp17'de FFT ile değişmeli — bunu izliyoruz
...
if __name__ == "__main__":
    ...
    for model_type in models_to_run:
        hop  = args.multihop_k if model_type == "fluid" else 1
        spec = USE_SPECTRAL     if model_type == "fluid" else False
        for seed in args.seeds:
            result = train_one(
                model_type=model_type, scale_cfg=sc, seed=seed,
                token_budget=token_budget, out_dir=out_dir,
                vocab_size=vocab_size, hop_k=hop, use_spectral=spec,
            )
            all_results.append(result)

    if IS_MASTER:
        master_print(f"\n{'='*60}")
        master_print("  EXP 17 SONUCLARI — Spectral Pressure")
        master_print(f"{'='*60}")
        master_print(f"  {'Run ID':<40} {'Best PPL':>10} {'MFLOP':>8}  Pressure")
        master_print(f"  {'-'*75}")
        for r in all_results:
            master_print(
                ...
                f"{'FFT-Helmholtz' if r.get('use_spectral') else 'cumsum/MHA'}"
            )

        # Exp16 karşılaştırması
        exp16_path = os.path.join(out_dir, "16_multihop_ns.json")
        if os.path.exists(exp16_path):
            try:
                with open(exp16_path, encoding="utf-8") as f:
                    exp16 = json.load(f)
                master_print(f"\n  [Exp16 vs Exp17]")
                master_print(f"  {'Run':<40} {'PPL':>10}")
                master_print(f"  {'-'*52}")
                for r in exp16:
                    master_print(f"  {r['run_id']:<40} {r['best_val_ppl']:>10.4f}  (exp16 cumsum)")
                for r in all_results:
                    tag = "FFT" if r.get("use_spectral") else "cumsum"
                    master_print(f"  {r['run_id']:<40} {r['best_val_ppl']:>10.4f}  (exp17 {tag})")
            except Exception:
                pass

        out_path = os.path.join(out_dir, "17_spectral_pressure.json")
