# ============================================================
# FluidLM — Multi-Hop NS Experiment (Exp 16)
# experiments/16_multihop_ns.py
#
# Araştırma Sorusu (RQ4):
#   Multi-scale causal gradient (k-hop), tek adımlık gradyana
#   göre NS routing'in representasyon gücünü artırıyor mu?
#
# Değişiklik (vs exp14):
#   causal_gradient: u[i] - u[i-1]  (1-hop, receptive field=1)
#     →
#   multihop_gradient: average of (u[i]-u[i-s])/s for s in [1,2,4,...,2^floor(log2(k))]
#   Örnek k=4: scales=[1,2,4] → her token 4 adım öncesini görebilir
#   Örnek k=8: scales=[1,2,4,8] → 8 adım
#   Hepsi O(L·T·D) — attention'ın O(L·T²·D)'si yok
#
# Kullanım:
#   python experiments/16_multihop_ns.py --multihop_k 4 --tokens 1e9
#   python experiments/16_multihop_ns.py --multihop_k 8 --tokens 1e9
#   python experiments/16_multihop_ns.py --multihop_k 1 --tokens 1e9  # ablasyon: k=1 = exp14
#
# Çıktı:
#   results/16_multihop_ns_k{K}_s{seed}_best.pt
#   results/16_multihop_ns.json
# ============================================================
...
parser = argparse.ArgumentParser(description="FluidLM Multi-Hop NS Experiment")
parser.add_argument("--multihop_k",  type=int,   default=4,
                    help="Multi-scale gradient max adım (2^floor(log2(k)) scales). "
                         "k=1 → tek adım (exp14 ablasyon). k=4 → [1,2,4]. k=8 → [1,2,4,8]")
parser.add_argument("--scale",       default="S",  choices=["S","M"],
                    help="Model ölçeği: S (~120M), M (~350M)")
parser.add_argument("--model",       default="fluid", choices=["fluid","gpt","both"],
                    help="Eğitilecek model (varsayılan: fluid)")
parser.add_argument("--tokens",      type=float, default=1e9,
                    help="Token budget. Varsayılan: 1B")
...
master_print(f"[Exp16] multihop_k={args.multihop_k}  "
             f"scales={[2**i for i in range(max(1, int(math.log2(max(1, args.multihop_k)))+1))]}")
...
if args.batch_tokens:
    BATCH_OVERRIDE = max(1, args.batch_tokens // args.seq_len)
else:
    BATCH_OVERRIDE = None
...
try:
    ...
    master_print("[WARNING] tiktoken bulunamadı — char-level fallback")
    ENC    = None
    VOCAB  = None
    encode = None

# ─────────────────────────────────────────────────────────────────────────────
# Veri Yükleme (exp14 ile aynı)
# ─────────────────────────────────────────────────────────────────────────────
...
class TokenStream:
    ...
    def _load(self, split: str):
        ...
        master_print(f"[Data] Shakespeare fallback  split={split}  tokens={len(ids):,}")
    ...
    def _next_from_stream(self, n_tokens: int) -> List[int]:
        ...
        while len(buf) < n_tokens:
            try:
                ...
                master_print("[Data] Stream bitti, yeniden baslatiliyor...")
                from datasets import load_dataset
                ds = load_dataset("Skylion007/openwebtext", split="train", streaming=True
...
def _hop_scales(k: int) -> List[int]:
    """[1, 2, 4, ..., 2^floor(log2(k))]  — en fazla k adıma kadar üstel ölçekler."""
    if k <= 1:
        return [1]
    n = int(math.log2(k))
    return [2**i for i in range(n + 1)]   # k=4 → [1,2,4]; k=8 → [1,2,4,8]

def multihop_gradient(u: torch.Tensor, k: int = 4) -> torch.Tensor:
    """
    Multi-scale causal gradient.
    k=1 : u[i] - u[i-1]  (exp14 ile aynı — ablasyon)
    k=4 : mean( (u[i]-u[i-1])/1, (u[i]-u[i-2])/2, (u[i]-u[i-4])/4 )
    k=8 : mean( ... + (u[i]-u[i-8])/8 )
    Hepsi sıfır-padding ile causal.
    """
    scales = _hop_scales(k)
    total  = torch.zeros_like(u)
    for s in scales:
        p     = F.pad(u, (0, 0, s, 0))[:, :-s, :]   # u[i-s]
        total = total + (u - p) / s
    return total / len(scales)

def multihop_laplacian(u: torch.Tensor, k: int = 4) -> torch.Tensor:
    """Multi-scale causal Laplacian: mean of 2nd-order diffs at each scale."""
    scales = _hop_scales(k)
    total  = torch.zeros_like(u)
    for s in scales:
        p     = F.pad(u, (0, 0, 2 * s, 0))
        lap_s = u - 2 * p[:, s:-s, :] + p[:, :-2*s, :]
        total = total + lap_s / (s * s)
    return total / len(scales)
...
# ─────────────────────────────────────────────────────────────────────────────
# FluidLayer v4 + multi-hop
# ─────────────────────────────────────────────────────────────────────────────

class FluidLayer(nn.Module):
    """
    NS timestep + MLP sublayer.
    v4 content-dependent speed + multi-hop gradient.
    """
    def __init__(self, d_model: int, hop_k: int = 4,
    ...
    def _rhs(self, u: torch.Tensor) -> torch.Tensor:
        ...
        div_adv = multihop_divergence(adv, self.hop_k)
...
class FluidLM(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, n_layers: int,
        ...
        self.layers     = nn.ModuleList([
            FluidLayer(d_model, hop_k=hop_k,
...
# ─────────────────────────────────────────────────────────────────────────────
# GPT Baseline (exp14 ile aynı — karşılaştırma için)
# ─────────────────────────────────────────────────────────────────────────────
...
class GPTBaseline(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, n_layers: int,
        ...
        self.drop_emb  = nn.Dropout(dropout)
        self.layers    = nn.ModuleList([
            GPTLayer(d_model, n_heads, mlp_ratio, dropout)
            for _ in range(n_layers)
        ])
        self.norm    = nn.LayerNorm(d_model)
        self.lm_head = nn.Linear(d_model, vocab_size, bias=False)
...
def flops_per_token(model_type: str, d: int, L: int, T: int,
                    vocab: int, mlp_ratio: int, hop_k: int = 4) -> dict:
    hidden    = d * mlp_ratio
    mlp_total = 2 * 2 * d * hidden * L

    if model_type == "fluid":
        n_scales = len(_hop_scales(hop_k))
        # W_q, W_k projections (v4)
        d_k = max(d // 8, 16)
        wqk_per_token = 2 * 2 * d * d_k
        # Multi-hop ops (n_scales iterations each): grad, laplacian, divergence, p_grad
        # Per scale: ~9d ops (grad+lap+adv+div+cumsum+p_grad+visc)
        ns_per_token  = wqk_per_token + d_k + n_scales * d * 9
        routing_total = ns_per_token * L
        routing_label = f"NS v4+multihop-k{hop_k} (scales={_hop_scales(hop_k)})"
        routing_complexity = "O(L·T·D·log(k))"
    else:
        n_heads  = d // 64
        head_dim = d // n_heads
...
def train_one(model_type: str, scale_cfg: ScaleConfig, seed: int,
    ...
    sc     = scale_cfg
    run_id = f"16_{model_type}_k{hop_k}_{sc.name}_s{seed}"
    master_print(f"\n{'='*60}")
    master_print(f"  RUN: {run_id}")
    master_print(f"  Scale: d={sc.d_model}, L={sc.n_layers}, seq={sc.seq_len}")
    master_print(f"  multihop_k={hop_k}  scales={_hop_scales(hop_k)}")
...
if __name__ == "__main__":
    ...
    if IS_MASTER:
        master_print(f"\n{'='*60}")
        master_print("  EXP 16 SONUCLARI — Multi-Hop NS")
        master_print(f"{'='*60}")
        master_print(f"  {'Run ID':<35} {'Best PPL':>10} {'MFLOP':>8} {'Routing'}")
        master_print(f"  {'-'*75}")
        for r in all_results:
            master_print(
                ...
                f"{r['flops_info']['routing_label']}"
            )

        # JSON kaydet
        out_path = os.path.join(out_dir, "16_multihop_ns.json")
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(all_results, f, indent=2, default=str)
        master_print(f"\n[JSON] Kaydedildi: {out_path}")

        # Exp14 ile karşılaştırma notu
        exp14_path = os.path.join(out_dir, "14_industrial_compare.json")
        if os.path.exists(exp14_path):
            try:
                with open(exp14_path, encoding="utf-8") as f:
                    exp14 = json.load(f)
                master_print(f"\n  [Exp14 vs Exp16 Karsilastirmasi]")
                master_print(f"  {'Run':<35} {'PPL':>10}")
                master_print(f"  {'-'*47}")
                for r in exp14:
                    master_print(f"  {r['run_id']:<35} {r['best_val_ppl']:>10.4f}  (exp14)")
                for r in all_results:
                    master_print(f"  {r['run_id']:<35} {r['best_val_ppl']:>10.4f}  (exp16)")
            except Exception:
                pass
